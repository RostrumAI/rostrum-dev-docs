...
| --- | --- | --- | --- |
| M2 | A workflow can execute locally | [E2.1](../epics/m2/e2-1-establish-daemon-network-boundary.md), [E2.2](../epics/m2/e2-2-execute-sequential-workflows.md), [E2.3](../epics/m2/e2-3-execute-conditional-workflows.md), [E2.4](../epics/m2/e2-4-execute-parallel-paths-and-joins.md), [E2.5](../epics/m2/e2-5-execute-bounded-loops.md), and [E2.6](../epics/m2/e2-6-complete-m2-conformance.md) | A caller invokes an exact publication through the Control API, disconnects, and later retrieves its progress, output, or failures after the daemon executes every workflow format v1 control-flow construct. Workflow-configured loop policies determine whether eligible iteration errors stop the run or remain in ordered results. The separately deployable services use private authenticated HTTP for commands and live run observation, and each accesses the same Postgres database through `packages/database`. E2.6 proves this with a shared workflow catalog and real services on separate network addresses. |
