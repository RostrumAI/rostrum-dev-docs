# Workflow format v1 specification
...
- the daemon, which executes publications;
- the Rostrum Cloud control plane, which reimplements the same contract.

A workflow document is a single JSON object in camelCase, optimized for machine generation and parsing. The primary authoring surface is a visual editor. The document carries no lifecycle fields: creation time, revision identity, publication number, and digest are assigned by the server during the lifecycle defined in [Draft and publication lifecycle](#draft-and-publication-lifecycle).
...
| --- | --- | --- | --- |
| `workflowFormatVersion` | string | **required** | The workflow format version. v1 is the literal `"v1"`. Rostrum selects the document schema and step-type registry from this value by exact match; there is no fallback. See [Format versioning and evolution](#format-versioning-and-evolution). |
...
| `type` | string | **required** | Step-type name. Selects the configuration schema and validation rules from the step-type registry for the declared workflow format version. |
...
A step type is a registry entry keyed by its `type` string. The entry contributes a JSON Schema 2020-12 fragment for the step's `config` object and any validation rules beyond the config schema. Rostrum validates a step's `config` against the schema registered for its `type` under the declared `workflowFormatVersion`.
...
| Publication | integer ≥ 1 | Server | Increments by 1 per successful publish |
...
- Publication re-runs validation on the stored content. Blocking findings reject the publish without creating anything. A valid revision is canonicalized once and stored as an immutable publication with its digest. The stored content is the canonical text; verification is `sha256(retrieved) == digest`.
- The response carries the workflow `id`, publication number, `workflowFormatVersion`, and digest.
...
| Edit a draft after publication | New revision; publications unchanged |
| Rewind to an earlier revision | A copy of the target becomes current; every revision stays in history; publications unchanged |
| Rewind past the newest publication's source | Succeeds — the publication's source revision stays retrievable |
| Rewind to the current revision | No-op |

### Rewind

Rewinding appends a copy of the earlier target revision as the newest revision and repoints the draft's current-revision pointer at that copy. The copy carries the target's exact bytes and findings snapshot and is stored with the `rewind` type; the target and every newer revision stay in history, so publications always keep their source revisions retrievable. Further saves create new revisions after the copy.
...
- **Metadata exclusion.** The top-level `name` and `description` members — the metadata-class fields in v1 — are removed from the parsed document before canonicalization. The digest therefore covers `workflowFormatVersion`, `id`, `inputs`, `steps`, and `conditionals`: the definitional content. A metadata-only edit leaves the digest unchanged. The publication's stored content is the full canonical document including metadata; only the digest excludes it.
...
The remaining invalid-shape examples each isolate one rule: a missing required field, an unknown top-level field, a malformed UUID, an empty `steps` array, a missing `workflowFormatVersion`, a `maxIterations` value below 1, a loop missing its `collection`, and a conditional missing its `default`:
